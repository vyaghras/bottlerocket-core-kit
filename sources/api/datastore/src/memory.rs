...
impl DataStore for MemoryDataStore {
        ...
        C: Fn(&mut Self, &Committed) -> Result<ConstraintCheckResult>,
    {
        ...
        let constraint_check_result = constraint_check(self, &pending)?;
        let approved_write = Option::<ApprovedWrite>::from(constraint_check_result);

        let mut pending_keys: HashSet<Key> = Default::default();
        // Remove anything pending for this transaction
        if let Some(approved_write) = approved_write {

            print!("Approved write data : {:?}", approved_write.settings);
            print!("Approved write metadata : {:?} end metadata ", approved_write.metadata);

            if !approved_write.settings.is_empty() {
                // Save Keys for return value
                pending_keys = approved_write.settings.keys().cloned().collect();
                print!("{:?}", approved_write.settings);
        ...
        print!("write completed {:?} end write", self.live);
...
mod test {
    ...
    fn constraint_check(
        ...
        committed: &Committed,
    ) -> super::Result<ConstraintCheckResult> {
