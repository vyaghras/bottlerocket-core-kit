...
impl DataStore for FilesystemDataStore {
        ...
        C: Fn(&mut Self, &Committed) -> Result<ConstraintCheckResult>,
    {
        ...
        let constraints_check_result = constraint_check(self, &pending)?;
        let approved_write = Option::<ApprovedWrite>::from(constraints_check_result);

        let mut pending_keys: HashSet<Key> = Default::default();

        if let Some(keys_to_write) = approved_write {
            trace!(
                "commit_transaction: transaction_metadata: {:?}",
                keys_to_write.metadata
            );

            // write all metadata.
            for (metadata_key, data_key, value) in keys_to_write.metadata {
                self.set_metadata(&metadata_key, &data_key, value, &Committed::Live)?;
            }

            let pending_data = keys_to_write.settings;
